...
const SHEETS_WEBAPP_URL = "https://script.google.com/macros/s/YOUR_DEPLOYMENT_ID/exec"; // Dán URL Web App sau khi deploy
...
(function(){
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    if(!phoneOK){ alert('Số điện thoại chưa đúng định dạng VN'); return; }
    if(payload.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.email)){ alert('Email chưa hợp lệ'); return; }
    if(!document.getElementById('consent').checked){ alert('Vui lòng đồng ý cho phép liên hệ'); return; }
